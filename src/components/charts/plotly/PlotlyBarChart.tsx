...
export function PlotlyBarChart({ data, title, className, style }: PlotlyBarChartProps) {
  ...
  const plotData = [
    {
      ...
      y: data.map(item => item.primaryProduct),
      type: 'bar',
    ...
    {
      x: data.map(item => item.name),
      y: data.map(item => item.secondaryProduct),
      type: 'bar',
  ...
  const handleClick = (event: any) => {
    if (event.points && event.points.length > 0) {
      ...
      setSelectedBar(data[point.pointIndex].name);
    }
  };
